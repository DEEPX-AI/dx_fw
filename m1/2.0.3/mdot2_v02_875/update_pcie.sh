...
function search_file() {
    ...
    cd "$directory" || { echo "Error: Cannot change to the specified directory."; return 1; }
...
function do_pcie_rescan() {
	local script_dir="$(realpath "${__base_dir}/../dx_rt/driver")"
	local pcie_rescan

	pcie_rescan=$(search_file "${script_dir}" "pcie_rescan.sh")
	if [ $? -ne 0 ]; then
        echo "Error: pcie_rescan.sh File not found."
	fi
